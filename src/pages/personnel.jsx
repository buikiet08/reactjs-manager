  ...
  return (
    <>
      <div className='flex items-center gap-4 mb-5'>
        <Link to={'/nhan-su'} className={`py-2 px-4 rounded-3xl bg-slate-400 text-white ${id_team === 0 && '!bg-[#1677ff]'}`}>Tất cả</Link>
        {
          listCategory.map((i, index) => {
            return (<Link to={generatePath('/nhan-su' + '?id_item=' + (index + 1))} className={`py-2 px-4 rounded-3xl bg-slate-400 text-white ${id_team == (index + 1) && '!bg-[#1677ff]'}`}>{i}</Link>)
          })
        }
      </div>
      <div className='flex flex-col gap-[1px] pb-[60px]'>
        <Row className='py-3 bg-[#1677ff]'>
          ...
          <Col className='text-center font-bold text-white' span={3}>Bộ phận</Col>
          <Col className='text-center font-bold text-white' span={3}>Cấp bậc</Col>
        ...
        </Row>
        {
          loading ? <Row className='h-[200px] flex justify-center items-center'><Spin /></Row> :
            data?.data.map((i, index) => (<PersonalItem key={index} index={index + 1} id={i.id} {...i} />))
        }
